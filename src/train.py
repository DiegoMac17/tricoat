    ...
    if ablation == 'full': 
        if tune:
            ...
            if config['alt']:
                ...
                # model = multi_attn_enc_concat_all_nomaxpool(config)Z4g%GSq&3$Y&@^X
    ...
    for epoch in range(1, 100): 
        ...
        for batch_idx, (_, _, clincal, target, gen, img, gene) in enumerate(train_loader):
            ...
            if ablation == 'full':
                ...
                train_loss = loss_joint + config['alpha'] * loss_aux_clin + config['beta'] * loss_aux_gen
        ...
        with torch.no_grad():
            for batch_idx, (_, _, clincal_val, target_val, gen_val, img_val, gene_val) in enumerate(val_loader, 0):
                ...
                if ablation == 'full':     
                    ...
                    severity_score, clin_aux, gen_aux, out_embedding, img_clin_attn, img_gen_attn, clin_gen_attn = model(clincal_val, img_val, gen_val, gene_val)
                elif ablation == 'img':
                ...
                if ablation == 'full':
                    ...
                    v_loss = loss_joint + config['alpha'] * loss_aux_clin + config['beta'] * loss_aux_gen
